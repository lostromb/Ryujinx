...
namespace Ryujinx.Graphics.GAL.Multithreading.Resources
{
    ...
    class ThreadedTexture : ITexture
    {
        ...
        {
            _renderer.New<TextureSetDataCommand>().Set(Ref(this), Ref(new ThreadedTextureData(data)));
            _renderer.QueueCommand();
        }

        public void SetData(PooledSpan data, int layer, int level)
        {
            _renderer.New<TextureSetDataSliceCommand>().Set(Ref(this), Ref(new ThreadedTextureData(data)), layer, level); // todo implement
            _renderer.QueueCommand();
        }
